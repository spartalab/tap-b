...
double bisection(network_type *network, double **direction, int iteration,
    ...
    for (k = 0; k < parameters->maxLineSearchIterations; k++) {
        displayMessage(DEBUG, "Testing %f of %d\n", lambda, parameters->maxLineSearchIterations);
        lambda = (lmax + lmin) / 2;
        der = 0;
        for (c = 0; c < network->numClasses; c++) {
//            changeFixedCosts(network, c);
    ...
    displayMessage(DEBUG, "Step size: %f (algo %d)\n", lambda);
...
//        changeFixedCosts(network, c);
    ...
    if (denom != 0) {
        ...
        /* Project to 0 if the alpha value is 0 */
        if (alpha < 0) alpha = 0;
    } else {
        alpha = 0;
    }
