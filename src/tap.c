...
/*
updateAllCosts recalculates and updates all link costs in the network,
according to costFunction, then adds in class-specific costs
*/
void updateAllCostsClass(network_type *network, int c) {
    for (i = 0; i < network->numArcs; i++)
      network->arcs[i].cost=network->arcs[i].calculateCost(&network->arcs[i], c);
}
